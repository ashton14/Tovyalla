...
function ContractPreview({ contractData, onClose, onGenerate }) {
  ...
  useEffect(() => {
    ...
    const { expenses, savedMilestones } = contractData

    // Generate milestones from current expenses
    const generatedMilestones = []
    let sortOrder = 0

    // 1. Initial Contract Fee
    const initialPrice = findSavedPrice(savedMilestones, 'initial_fee')
    generatedMilestones.push({
      id: `new-initial-fee`,
      name: 'Initial Contract Fee',
      costAmount: 1000,
  ...
  // Calculate totals
  const totalCost = milestones.reduce((sum, m) => sum + m.costAmount, 0)
  const totalCustomerPrice = milestones.reduce((sum, m) => sum + m.customerPrice, 0)
  const profit = totalCustomerPrice - totalCost
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      const customerPaymentSchedule = milestones.map(m => ({
        description: m.name,
      ...
      const modifiedContractData = {
        ...
        customerGrandTotal: totalCustomerPrice,
      }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Preview Scope & Pricing</h2>
